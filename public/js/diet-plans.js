...
(function() {
  ...
  function showResults(calories) {
    results.innerHTML = `<h3>Your basal metabolic rate (BMR) is: <strong>${(calories).toFixed(2)} </strong> calories a day.</h3>`;
    results.style.display = '';
    // form.style.display = 'none';
    errors.style.display = 'none';

    $('.card').each(function(){
      var cardUrl = $(this).find('.btn').attr('href');
      
      //concat the cardUrl and add calories to the end of the URL
      var finalUrl = cardUrl + calories;

      // now we apply the URL to the button
      $(this).find('.btn').attr('href',finalUrl);
    });
  ...
  function submitHandler(e) {
    ...
    if(form.gender.value == 'Female') {
     ...
     //remove disabled class from buttons in cards
