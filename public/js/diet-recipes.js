$(document).ready(function () {
  ...
  let recipe = $(".page-title").text().trim();
  //console.log('urlParams.get("cal")', urlParams.get("cal"));
  let userCalories = urlParams.get("cal");
  ...
  $.ajax({
    ...
    complete: function (response) {
      console.log("response in complete", response);
      ...
      for (var i = 0; i < 3; i++) {
        ...
        if (calories < mealCalories) {
          ...
          var badHtml =
            ...
            '<div class="card-body">' +
            '<a href="' +
            recipeResults.shareAs +
            "\"><img class=\"recipe-image\" src=\"" +
            recipeResults.image +
            '"></a>' +
            '<p class="recipe-name"></p>' +
            '<h2>Calories: <span class="recipe-calories">' +
            Math.round(recipeResults.calories) +
            "</span></h2>" +
            "</div></div></div>";
